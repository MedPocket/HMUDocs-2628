...
```
[Thao tác SPSS]
Data -> Select Cases -> If condition is satisfied -> [If...] -> tuoi >= 60 AND hba1c > 7.0 -> Continue -> OK
```
...
- **Công thức**: $\text{BMI} = \frac{\text{Cân nặng (kg)}}{\left(\text{Chiều cao (m)}\right)^2} = \frac{\text{can\_nang}}{\left(\frac{\text{chieu\_cao}}{100}\right)^2}$
- **Thao tác**:
...
```
[Thao tác SPSS]
Transform -> Compute Variable -> Target Variable: BMI
-> Numeric Expression: can_nang / ((chieu_cao / 100)**2) -> OK
```
...
> ⚠️ **Quy tắc vàng**: Luôn chọn `Recode into Different Variables` (Mã hóa thành biến mới) để bảo toàn biến gốc ban đầu. Không dùng `Recode into Same Variables` trừ khi thực sự cần đè lên dữ liệu cũ.

### Ví dụ: Phân nhóm chỉ số BMI theo chuẩn WHO Châu Á:
- $\text{BMI} < 18.5$: Gầy (Mã = 1)
- $18.5 \le \text{BMI} < 23.0$: Bình thường (Mã = 2)
- $23.0 \le \text{BMI} < 25.0$: Thừa cân (Mã = 3)
- $\text{BMI} \ge 25.0$: Béo phì (Mã = 4)
...
4. Bấm nút **Old and New Values...**:
   - Range `Lowest through 18.49` $\rightarrow$ New Value = `1` $\rightarrow$ Add
   - Range `18.50 through 22.99` $\rightarrow$ New Value = `2` $\rightarrow$ Add
   - Range `23.00 through 24.99` $\rightarrow$ New Value = `3` $\rightarrow$ Add
   - Range `25.00 through highest` $\rightarrow$ New Value = `4` $\rightarrow$ Add
