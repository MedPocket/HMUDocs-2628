...
![Bivariate Correlations Matrix Output](_images/img-6-hoi-quy-1.png)
...
3. Tại mục Correlation Coefficients:
   - Tích chọn Pearson (nếu phân phối chuẩn).
   - Tích chọn Spearman (nếu phân phối không chuẩn).
4. Bấm OK.
...
Hồi quy tuyến tính biểu diễn mối quan hệ phụ thuộc giữa một biến phụ thuộc định lượng liên tục (Y) theo một hoặc nhiều biến độc lập (X_1, X_2, ...).
...
1. Hệ số xác định R^2 (R Square): Cho biết tỷ lệ phần trăm sự biến thiên của Y được giải thích bởi mô hình các biến độc lập X. (Ví dụ: R^2 = 0.35 -> X giải thích được 35% sự biến thiên của Y).
...
logit(P) = ln(P / (1 - P)) = beta_0 + beta_1 * X_1 + beta_2 * X_2 + ... + beta_k * X_k
...
4. Nếu biến độc lập là biến định tính phân nhóm -> Bấm Categorical...:
   - Đưa biến định tính vào Categorical Covariates.
   - Chọn nhóm tham chiếu (Reference Category): First hoặc Last -> Bấm Change -> Continue.
5. Bấm Options...: Tích chọn CI for exp(B): 95%.
6. Bấm Continue -> OK.
...
- Cột Exp(B): Tỷ số chênh Odds Ratio hiệu chỉnh (Adjusted OR).
- Cột 95% C.I. for EXP(B): Khoảng tin cậy 95% của OR hiệu chỉnh.

Phiên giải ví dụ: Bệnh nhân có hút thuốc lá có nguy cơ mắc bệnh tăng huyết áp cao gấp 2.85 lần so với nhóm không hút thuốc sau khi đã hiệu chỉnh theo tuổi và chỉ số BMI (Adjusted OR = 2.85; 95% CI: 1.42 - 5.71; p = 0.003 < 0.05).
