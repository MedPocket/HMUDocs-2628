...
| (Hiển thị dữ liệu thực tế)         | (Hiển thị cấu trúc thuộc tính)  |
| - Mỗi hàng = 1 Quan sát (Case)    | - Mỗi hàng = 1 Biến số (Variable)|
| - Mỗi cột  = 1 Biến số (Variable) | - Định nghĩa tên, kiểu, nhãn...  |
+------------------------------------+----------------------------------+
```
...
- **Cấu trúc hàng**: Mỗi hàng đại diện cho một đối tượng nghiên cứu hay một bản ghi (Case/Observation), ví dụ: bệnh nhân $N_1, N_2, \dots$.
...
2. **Ordinal (Thang đo thứ tự)**: Các giá trị phân loại có mối quan hệ thứ tự hơn kém rõ rệt (Ví dụ: Mức độ hài lòng: 1-Rất không hài lòng $\rightarrow$ 5-Rất hài lòng; Giai đoạn ung thư: I, II, III, IV).
...
```mermaid
graph TD
    A[Xây dựng Phiếu thu thập số liệu / Bệnh án nghiên cứu] --> B[Khai báo biến trong Variable View]
    B --> C[Gán mã hóa Values & Missing]
    C --> D[Nhập liệu trong Data View]
    D --> E[Kiểm tra làm sạch dữ liệu & Sửa lỗi logic]
    E --> F[Lưu tệp tin dưới dạng .SAV]
```
...
3. Trong hộp thoại **Read Excel File**:
   - Tích chọn: $\checkmark$ **Read variable names from the first row of data** (Đọc tên biến từ hàng đầu tiên).
...
```
[Thao tác SPSS]
File -> Import Data -> Excel -> Chọn file -> [x] Read variable names from first row -> OK
```
