...
```
          ...
          +----------------------------+----------------------------+
          |                                                         |
  BIẾN ĐỊNH TÍNH (Categorical)                            BIẾN ĐỊNH LƯỢNG (Quantitative)
          |                                                         |
  +-------+-------+                                         +-------+-------+
  |               |                                         |               |
Danh nghĩa     Thứ tự                                    Rời rạc         Liên tục
(Nominal)     (Ordinal)                                 (Discrete)     (Continuous)
VD: Giới tính, VD: Giai đoạn ung thư                     VD: Số con,     VD: Tuổi, Cân nặng,
    Tỉnh thành     Mức độ đau                               Số ngày nằm viện    Huyết áp
```
...
- **Biểu đồ hình tròn (`Pie Chart`)**: Thích hợp khi biến định tính có ít phân loại ($\le 5$ nhóm) và tổng các tỷ lệ bằng $100\%$.

### 2.3. Lệnh SPSS
- **Thực hiện**: `Analyze > Descriptive Statistics > Frequencies...`
- Chuyển các biến định tính cần mô tả vào ô **Variable(s)**.
- Đảm bảo tích chọn $\checkmark$ **Display frequency tables**.
- Vào nút **Charts...**: Chọn `Bar charts` hoặc `Pie charts` $\rightarrow$ Choose `Percentages` $\rightarrow$ **Continue** $\rightarrow$ **OK**.
...
1. **Về mặt chỉ số thống kê**:
   ...
   - System Skewness (Độ lệch) và Kurtosis (Độ nhọn) nằm trong khoảng từ $-1.0$ đến $+1.0$ (hoặc chỉ số Skewness/SE_Skewness nằm trong $[-3, +3]$).
2. **Về mặt biểu đồ**:
...
3. **Kiểm định giả thuyết phân phối chuẩn**:
   - Kiểm định Kolmogorov-Smirnov (khi cỡ mẫu $N \ge 50$) hoặc Shapiro-Wilk (khi cỡ mẫu $N < 50$).
   - Nếu $p > 0.05$: Chấp nhận giả thuyết $H_0 \rightarrow$ Biến số có phân phối chuẩn.
   - Nếu $p \le 0.05$: Bác bỏ $H_0 \rightarrow$ Biến số có phân phối KHÔNG chuẩn.
...
| **Phân phối Không chuẩn** | Trung vị (`Median`) | Khoảng tứ phân vị (`IQR`) / Min-Max | $Median (IQR)$ (Ví dụ: $5.8 (4.2 - 8.1) \text{ mg/dL}$) |

### 3.3. Biểu đồ mô tả biến định lượng
- **Histogram (Cột liên tục)**: Thể hiện hình dạng phân phối.
- **Boxplot (Biểu đồ hộp & râu)**: Giúp phát hiện trung vị, khoảng tứ phân vị và các giá trị ngoại lệ (Outliers - ký hiệu $\circ$ hoặc $\star$).
...
- Bấm **Plots...**: Tích chọn $\checkmark$ **Histogram** và $\checkmark$ **Normality plots with tests**.
- Bấm **Continue** $\rightarrow$ **OK**.

```
[Thao tác SPSS]
Analyze -> Descriptive Statistics -> Explore -> Dependent List: [Tuoi, BMI]
-> Plots... -> [x] Histogram, [x] Normality plots with tests -> OK
```
...
- **Thao tác SPSS**: `Analyze > Descriptive Statistics > Crosstabs...`
  ...
  - Vào **Cells...**: Chọn $\checkmark$ **Row** (Tỷ lệ phần trăm theo dòng) $\rightarrow$ **Continue** $\rightarrow$ **OK**.
